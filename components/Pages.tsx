    ...
    >
      ...
      {!props.isSingle && (
        <IconButton
          ...
          color="neutral"
          size="sm"
          sx={{ ml: 1, opacity: { xs: 1, sm: 0 }, transition: 'opacity 0.3s', ...(props.isActive ? { color: 'white' } : {}) }}
...
export function PagesMenu(props: { conversationId: string | null; pagesMenuAnchor: HTMLElement | null; onClose: () => void }) {
  ...
  const { setActiveConversationId, createConversation, deleteConversation, setActiveConversation } = useChatStore(
    (state) => ({
  ...
  const handleConfirmedDeleteConversation = () => {
    if (hasChats && deleteConfirmationId) {
      ...
      } else deleteConversation(deleteConfirmationId);
  ...
  const NewPrefix = maxReached && (
    <Tooltip title={`Maximum limit: ${MAX_CONVERSATIONS} chats. Proceeding will remove the oldest chat.`}>
      <span>⚠️ </span>
    </Tooltip>
  );
    ...
    <>
      ...
      >
        ...
        <MenuItem onClick={handleNew}>
          ...
          <Typography>{NewPrefix}New</Typography>
        </MenuItem>
